        ...
        if (pluginLocation) {
          define([pluginLocation], function(plugin) {
            ...
            ajax.get(buildUrl(resource), null, function(response) {
              // process file, giving callback in case of async
              var module = plugin.processFile(response, function(module) {
                register(module, location);
              });

              // if sync, just call the function
              if (module) {
                register(module, location);
              }
            }, requestFormat);
